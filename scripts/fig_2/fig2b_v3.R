...
v_53BP1 <- ggplot(j_fil_Jur, aes(x= treat, y= spot_int53BP1_sum +1, fill= treat)) +
  ...
  ggtitle("Jurkat") + 
  annotate("text", x=1.50, y=500000, label = "33.4%", size = 6) +  #untreated and high
  annotate("text", x=1.50, y=10, label = "66.7%", size = 6) + #untreated and low 
  annotate("text", x=2.43, y=500000, label = "69.0%", size = 6) + #ETP and high 
  annotate("text", x=2.43, y=10, label = "31.0%", size = 6)  #EtP and low 
...
my_comparisons = list( c("ETP", "DMSO") )
stat_compare_means(comparisons = my_comparisons)
...
fig2b_bjab <- v_53BP1 + scale_fill_brewer(palette = "BuGn") + 
  stat_compare_means(comparisons = my_comparisons, label = "p.signif") + 
  theme_bw(base_size = 20) + 
  scale_x_discrete(labels = c("DMSO"= "DMSO", "ETP"= "ETP")) + 
  theme(axis.text = element_text(size = 15), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), axis.title.x = element_blank())
