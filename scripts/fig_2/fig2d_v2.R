...
X_122_jurkat <- read_excel("data/raw-data_v1/fig_2/190221-EXP122-DoseResponse-Jurkat.xlsx")
X_119_BJAB <- read_excel("data/raw-data_v1/fig_2/190218-119-BJAB.xlsx")
X_122_THP1 <- read_excel("data/raw-data_v1/fig_2/190218-EXP122-DoseResponse-THP1.xlsx")
...
j_fil_BJAB <- X_119_BJAB %>%
  filter(treat %in% c("DMSO", "ETP"))

#reorder groups 
j_fil_BJAB$treat <- factor(j_fil_BJAB$treat, levels=c("DMSO", "ETP"))
...
fig2d_jurkat <- Jurkat_confilled + theme_classic(base_size = 20) + theme(axis.text = element_text(size = 15), legend.position = "bottom") 
...
fig2d_bjab <- BJAB_confilled + theme_classic(base_size = 20) + theme(axis.text = element_text(size = 15))
...
fig2d_thp1 <- THP1_confilled + theme_classic(base_size = 20) + theme(axis.text = element_text(size = 15), legend.position = "bottom") 
